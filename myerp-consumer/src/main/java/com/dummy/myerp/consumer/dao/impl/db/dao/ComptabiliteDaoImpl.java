...
public class ComptabiliteDaoImpl extends AbstractDbConsumer implements ComptabiliteDao {
    ...
    /** SQLgetValeurJournalComptable */
    private static String SQLgetValeurJournalComptable;
    public void setSQLgetValeurJournalComptable(String pSQLgetValeurJournalComptable) {
        SQLgetValeurJournalComptable = pSQLgetValeurJournalComptable;
    }

    /** SQLupdateValeurJournalComptable */
    private static String SQLupdateValeurJournalComptable;
    public void setSQLupdateValeurJournalComptable(String pSQLupdateValeurJournalComptable) {
        SQLupdateValeurJournalComptable = pSQLupdateValeurJournalComptable;
    }

    /** SQLgetValeurJournalComptable */
    private static String SQLcheckCodeJournalComptable;
    public void setSQLcheckCodeJournalComptable(String pSQLcheckCodeJournalComptable) {
        SQLcheckCodeJournalComptable = pSQLcheckCodeJournalComptable;
    }

    private static String SQLinsertNewValeurJournalComptable;
    public void setSQLinsertNewValeurJournalComptable(String pSQLinsertNewValeurJournalComptable) {
        SQLinsertNewValeurJournalComptable = pSQLinsertNewValeurJournalComptable;
    }
    ...
    @Override
    public int getValeurJournalComptableByFeatures(String codeJournal, int year) throws NotFoundException {
        ...
        vSqlParams.addValue("jcode", codeJournal);
        vSqlParams.addValue("year", year);
        int valeurJC;
        try{
            valeurJC = vJdbcTemplate.queryForObject(SQLgetValeurJournalComptable, vSqlParams, Integer.class);
        } catch(Exception e){
            throw new NotFoundException("Aucun journal comptable n'a été trouvé pour le code journal : " + codeJournal + " et pour l'année " + year);
        }
        return valeurJC;
    }

    @Override
    public void updateSequenceJournalComptable(String codeJournal, int year, int lastValeur) throws NotFoundException {
        NamedParameterJdbcTemplate vJdbcTemplate = new NamedParameterJdbcTemplate(this.getDataSource(DataSourcesEnum.MYERP));
        MapSqlParameterSource vSqlParams = new MapSqlParameterSource();
        vSqlParams.addValue("jcode", codeJournal);
        vSqlParams.addValue("year", year);
        vSqlParams.addValue("valeur", lastValeur);
        if(lastValeur>1){
            try{
                vJdbcTemplate.update(SQLupdateValeurJournalComptable, vSqlParams);
            } catch(Exception e){
                throw new NotFoundException("Aucun journal comptable n'a été trouvé pour le code journal : " + codeJournal + " et pour l'année " + year);
            }
        }else {
                vJdbcTemplate.update(SQLinsertNewValeurJournalComptable, vSqlParams);
        }
    }

    @Override
    public boolean isJournalCodeExiste(String codeJournal) {
        NamedParameterJdbcTemplate vJdbcTemplate = new NamedParameterJdbcTemplate(this.getDataSource(DataSourcesEnum.MYERP));
        MapSqlParameterSource vSqlParams = new MapSqlParameterSource();
        vSqlParams.addValue("jcode", codeJournal);
        String checkCode = "";
        if(codeJournal != null && "".equals(codeJournal) == false){
            try{
                checkCode = vJdbcTemplate.queryForObject(SQLcheckCodeJournalComptable, vSqlParams, String.class);
            } catch(Exception e){
                checkCode = "";
            }
        }
    ...
    /** SQLgetListEcritureComptableDate */
    private static String SQLgetListEcritureComptableDate;
    public void setSQLgetListEcritureComptableDate(String pSQLgetListEcritureComptable) {
        SQLgetListEcritureComptableDate = pSQLgetListEcritureComptable;
    }
    ...
    public List<EcritureComptable> getListEcritureComptable(String dateFromWhenEngFormat, String dateToWhenEngFormat) throws NotFoundException{
        NamedParameterJdbcTemplate vJdbcTemplate = new NamedParameterJdbcTemplate(this.getDataSource(DataSourcesEnum.MYERP));
        MapSqlParameterSource vSqlParams = new MapSqlParameterSource();
        vSqlParams.addValue("date1", dateFromWhenEngFormat);
        vSqlParams.addValue("date2", dateToWhenEngFormat);
        EcritureComptableRM vRM = new EcritureComptableRM();
        List<EcritureComptable> vList;
        try{
            vList = vJdbcTemplate.query(SQLgetListEcritureComptableDate, vSqlParams, vRM);
        } catch (EmptyResultDataAccessException vEx){
            throw new NotFoundException("Aucune écriture comptable n'a été trouvée entre les dates " + dateFromWhenEngFormat + " et " + dateToWhenEngFormat);
        }
        if(vList.size() == 0){
            throw new NotFoundException("Aucune écriture comptable n'a été trouvée entre les dates " + dateFromWhenEngFormat + " et " + dateToWhenEngFormat);
        }
